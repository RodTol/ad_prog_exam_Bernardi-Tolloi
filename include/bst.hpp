...
* compare operator and a head. The first one is the relation 
* that will decide where a new node will be inserted while
* the second one is an unique pointer to the first node of 
* the tree. 
*
* Date: May, 2021.
*/
...
public:
    ...
    */
    ...
    /**Find:
     * It looks for a node with key equal to the input. If
    ...
    */
    ...
    /** Find:
     * It looks for a node with key equal to the input. If
     * it succeeds, it returns a const_iterator to the node, otherwise,
     ...
     * boolean and an iterator. The boolean is false if 
    ...
    */
    ...
    /** put-to:
     * Custom overloading of the put-to operator that will 
    ...
    */
     ...
     * balanced. To achieve this, it will start by inserting the
     ...
     * inserting an another node. By iterating this process
    ...
    */
     ...
     * of the subtree of an input node.
    */
    ...
    /**erase:
     * This function's purpouse is to remove a node with key 
     * equal to the the input (if it exists). The first step is
     * to check if this node exists. There are 3 cases: the node
     * has no child, has one child (left or right) or has 
     * both children. For the first case the function simply removes
    ...
    void erase(const key_type& x) {
        if (!_find(x))
        {
            ...
            else if ( (_find(x)->left) &&  (_find(x)->right))
            {
                ...
                leftomost node of its right subtree. So, I am again in one of 
                the cases above*/
